...
function getPeersFromDependencies(dependenciesObj, referencePath) {
    ...
    return Promise.all(dependencyKeys.map((key) => {
        return new Promise((success, reject) => {
            const path = referencePath + "/node_modules/" + key;
            return readPackageFromPath(path).then((depPackage)=>{
                try {
                    var newDependencies = {}
                    if(depPackage.peerDependencies) {
                        Object.keys(depPackage.peerDependencies).forEach((peerDependency)=>{
                            newDependencies[peerDependency] = depPackage.peerDependencies[peerDependency];
                        });
                    }
...
function saveDependencies(newDependencies, path) {
    return new Promise((success, reject) =>{
        ...
        return readPackageFromPath(path).then((package)=>{
            ...
            if(doSave) {
                return savePackage(package, path).then(
...
function promotePeerDependencies(path) {
    ...
    .then(
        (dependencies)=>{return getPeersFromDependencies(dependencies)}
    ).then(
        (newDependencies) => {return saveDependencies(newDependencies, path)}
    )
}
